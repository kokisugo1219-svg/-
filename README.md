# SPI言語トレーニング

SPIテストセンターの「言語」分野に特化した、自分専用の学習用Webアプリです。
ビルド不要の静的HTML/CSS/JavaScriptのみで動作します。

## 使い方

ローカルでブラウザから `index.html` を直接開くか、簡易サーバーを立てて開いてください。

```bash
python3 -m http.server 8000
# ブラウザで http://localhost:8000 を開く
```

GitHub Pages で公開する場合は、リポジトリの Settings > Pages でこのブランチ（または main）を
公開ソースに設定すれば、そのままアクセスできます。

## 機能

- 出題分野: 語句の意味 / 語句の用法 / 二語の関係 / 空欄補充 / 文の並び替え（計689問）
- 分野を選ぶと専用画面に遷移する二段構成。そこで以下のどちらかを選んで出題開始する
  - 20問ずつの固定ブロック。ブロックを選ぶとその20問がそのまま出題され、同じブロックを
    繰り返し周回できる（ブロックごとの正答率も表示）
  - 5問・10問・20問・全問から選ぶランダム出題
- 正解・不正解を即時フィードバック＋解説付きで表示
- 正誤履歴を `localStorage` に保存し、分野ごとの正答率を表示
- 直近で間違えた問題だけを出題する「苦手問題の復習」モード
- 単語帳モード（`js/tango.js`、413語）。カテゴリ・「まだ覚えていない語句のみ」で絞り込み、
  フリップ式カードで暗記状況を記録できる

## 問題データについて

`js/questions.js` の問題のうち `imp-` から始まるIDのものは、過去問集（xlsxファイル）を精査・変換して
取り込んだもの。選択肢が元データに無かった問題は、関連語句からダミー選択肢を自動生成している。
本文が欠落していて解答不能な「長文理解」問題や、レビューで問題自体が破綻していると判定されたものは
取り込みから除外済み。

## 問題を追加・編集する

問題データは `js/questions.js` の `QUESTIONS` 配列にあります。1問は以下の形式です。

```js
{
  id: 'nigo-021',                // ユニークなID
  category: 'nigo',              // 'goi_imi' | 'goi_yoho' | 'nigo' | 'kuran' | 'narabikae'
  instruction: '設問の指示文',
  promptHTML: '出題内容(HTML可)',
  choices: ['選択肢1', '選択肢2', '選択肢3', '選択肢4', '選択肢5'],
  answerIndex: 0,                // choices内の正解のインデックス
  explanation: '解説文',
}
```

配列に追記するだけで、アプリ側の分野一覧・出題・復習機能に自動的に反映されます。

## デプロイ時の注意（キャッシュ対策）

`index.html` のCSS/JS読み込みには `?v=N` を付けている。JS/CSSを変更してデプロイするたびに
この数字を増やすこと。付けないと、スマホのブラウザに古いJS/CSSがキャッシュされたまま新しい
`index.html` だけ読み込まれ、要素が噛み合わずに動作がおかしくなることがある。
